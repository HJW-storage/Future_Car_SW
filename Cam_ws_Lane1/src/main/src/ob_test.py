...
class Clustering:
    def __init__(self):
        ...
        self.limit_range = 1.4
        self.limit_degree = 60
        ######################################################
    ...
    ################################################################################################
    def avoidance(self, centroid_list, lane_idx):
        ...
        self.fsm.transition(detected_obstacle, lane_idx)
        # self.fsm.transition(centroid_list)
        
        # 상태 기반 회피
        if self.fsm.current_state == "AvoidLeft":
            # angle = min(-20, -(angle))  # 왼쪽으로 최소 10도 이상 회피
            angle = -20

        elif self.fsm.current_state == "AvoidRight":
            # angle = max(20, angle)  # 오른쪽으로 최소 10도 이상 회피
            angle = 20
        ############################################################
    ...
    def process(self, msg, img, lane_idx):

        # os.system("clear")
        coordinate = self.coordinate_transform(msg)
        centroid_list = self.clustering(coordinate)
        self.steering_angle = self.avoidance(centroid_list, lane_idx)
