...
class LaneDetector:
    ...
    def sliding_window(self, img, nwindows=7, margin=30, minpix=45, draw_windows=False):
        ...
        for window in range(nwindows):
            ...
            if window == 0:
                ...
                # if rightx_current > self.camera.WIDTH - 30:
                #     rightx_current = self.camera.WIDTH - 30                
    ...
    def get_angle(self, path_x, path_y, left_lane_detected, right_lane_detected):
        ...
        # 차선 하나라도 인식될 경우
            ...
            # making jiwhan
            self.has_switched = False # 다시 차선이 검출된 경우 부호 바꾸기를 초기화
            # making jiwhan
            ...
            ## persuit
            Ld = math.sqrt(((point_a[0] - point_b[0]) ** 2) + ((point_a[1] - point_b[1]) ** 2))

            Alpha = np.polyfit(path[:, 1], path[:, 0], deg = 1)[0] * 1000

            # self.real_angle=math.atan((2*self.xycar_length*np.sin(Alpha))/Ld)
            ##
    ...
    def process(self, origin_img):
        ...
        points = np.array([(100, 410), (120, 350), (560, 350), (580, 410)], dtype=np.int32)
        combined_img = cv2.polylines(combined_img, [points], True, (255, 255, 0), 1)
        combined_img = cv2.line(combined_img,(340, 350), (340, 410),(255,255,0),1)
