...
class Clustering:
    ...
    def coordinate_transform(self, distance):
        ...
        # np.concatenate : 배열 합치기 (https://pybasall.tistory.com/33)
    ...
    def value_handling(self, data):
        # inf값을 NaN으로 대체
        data[np.isposinf(data)] = np.NaN    # NaN(Not a Number) : (https://tempdev.tistory.com/54)
        data[np.isneginf(data)] = np.NaN    # https://numpy.org/doc/stable/reference/generated/numpy.isneginf.html
       
        # x좌표가 0.30m 이하인 점의 인덱스를 찾아 그 부분만 살림
        condition = np.where(np.abs(data[:, 1]) < 0.30) # 배열의 모든 행에서 두 번째 열의 값
        # https://stackoverflow.com/questions/58958512/slicing-what-does-1-and-1-mean
